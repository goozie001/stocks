...
class LoginActivity : AppCompatActivity(), LoaderCallbacks<Cursor> {
    ...
    private fun mayRequestContacts(): Boolean {
        ...
        if (shouldShowRequestPermissionRationale(READ_CONTACTS)) {
            val tempEmail: View? = mEmailView as? View
            if (tempEmail is View) {
                Snackbar.make(tempEmail, R.string.permission_rationale, Snackbar.LENGTH_INDEFINITE).setAction(android.R.string.ok, OnClickListener { requestPermissions(arrayOf(READ_CONTACTS), REQUEST_READ_CONTACTS) })
            }
        } else {
